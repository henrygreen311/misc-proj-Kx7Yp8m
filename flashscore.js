...
(async () => {
    ...
    for (const div of matchDivs) {
        ...
        if (!id || uniqueMatches.has(id)) continue;
        uniqueMatches.add(id);

        // Check if the match is upcoming by verifying the score div
    ...
    console.log(`Total unique upcoming matches found: ${upcomingMatches.length}`);
    ...
    for (const match of upcomingMatches) {
        ...
        console.log(`Processing H2H data for: ${match}`);
        
        // Click on the first upcoming match
        await page.click(`div.event__match.event__match--withRowLink.event__match--twoLine:has-text("${match}")`);
        ...
        if (!matchPage) {
            console.log("Failed to detect new tab. Skipping this match.");
            continue;
        }
        ...
        try {
            ...
            console.log("H2H tab not found. Skipping this match.");
            continue;
        }
        ...
        for (let i = 0; i < Math.min(3, h2hSections.length); i++) {
            ...
            const spanElements = await h2hSections[i].$$('span[data-testid="wcl-scores-overline-02"]');
            ...
            // Extract previous match results
        ...
        console.log(`Completed processing H2H for: ${match}`);
    }
